[설계서](../../../README.md) › [Logical-Viewpoint](../../README.md) › [Components](../Components.md) › Popup
...
| 변경 반영 요청 | 열려 있는 대시보드 탭 | 새 탭을 만들지 않고 그 탭으로 이동하며 바뀐 시청 목록을 전달. **대시보드를 여는 조작에서만 내보낸다** |
...
- ⚠️ 시청 목록을 저장소에 쓰는 것만으로는 **대시보드가 알지 못한다.** 열려 있는 대시보드 탭에 직접 알려야 그 자리에서 바뀐다.
- ⚠️ 그 알림은 **대시보드를 여는 조작에서만 내보낸다.** 목록을 고치는 조작들은 저장만 하고 알리지 않는다. 목록을 손보는 동안 보던 방송이 끊기지 않게 하려는 것이므로, 다른 조작에서 알림을 함께 내보내면 이 약속이 깨진다.
  - 그동안 팝업과 대시보드가 서로 다른 내용을 보여준다. 이것을 사용자에게 따로 알리지 않기로 했다.
