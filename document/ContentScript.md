...
**책임**
Dashboard가 만든 방송 화면 안에 들어가, 딜레이 측정·넓은 화면 자동 전환·채팅 접기·광고 건너뛰기를 수행한다. 음량 제어 기능도 갖고 있으나 지금은 쓰이지 않는다. SOOP 화면에서는 고화질 연결 안내 팝업 닫기도 함께 처리한다.
...
**주의**
- ⚠️ 음량을 다루는 기능(지정·잠금·보고)이 남아 있지만 **지금은 Dashboard가 쓰지 않는다.** 소리는 방송 페이지의 조절 기능에 맡기기로 했기 때문이다. 음량 크기를 덮어쓰면 방송 페이지가 그것을 채널별 상태로 저장해, 다시 불러올 때 페이지가 보여주는 상태와 실제 소리가 어긋난다. 다시 쓰게 될 때 이 점을 먼저 따져야 한다.
