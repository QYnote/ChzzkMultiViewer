[설계서](../README.md) › Changelog

# Changelog

> **다루는 내용:** 버전별 수정 내역
> **갱신 트리거:** 새 버전이 배포될 때

## 본문

### v3.1.1 (현재 배포 버전)

- 팝업에서 목록을 바꿔도 **보던 방송이 끊기지 않음.** 저장한 조합을 불러오거나 받은 글로 덮어써도 대시보드는 그대로 있다가, `멀티뷰 대시보드 열기`를 누를 때 한꺼번에 바뀜
- 치지직 방송을 좁은 칸에 넣으면 스크롤 막대가 생기던 문제 수정. 칸이 좁을 때는 방송 화면 전체가 비율 그대로 줄어들어 들어감
  - 칸 크기에 따라 옆 칸과 화면 배치가 어긋나 보이던 것도 함께 완화됨

### v3.1.0

**시청 목록을 이름 붙여 저장하고 통째로 갈아타기**

- 지금 보고 있는 조합에 이름을 붙여 저장해 두고, 나중에 한 번에 불러올 수 있음. 여러 조합을 만들어 두고 오갈 수 있음
- 저장한 조합을 글로 만들어 남에게 보내고, 받은 글을 붙여넣어 그대로 볼 수 있음
- 불러오면 지금 시청 목록을 덮어씀. 확인을 한 번 받으며, 담긴 채널이 지금과 같으면 보던 방송이 끊기지 않음
- 저장하는 것은 채널 목록뿐. 화면 배치는 채널마다 따로 기억되므로 담지 않아도 유지됨

**팝업 화면 개편**

- 시청목록 탭의 위아래 두 단 탭을 없애고 한 화면으로 합침. `스트리머 추가`를 보려고 탭을 오갈 필요가 없어짐
- 오른쪽 칸에서 `채널 추가` · `즐겨찾기` · `저장 목록`을 골라 봄. 팝업을 열면 `채널 추가`가 먼저 보임
- 채널 추가 화면에 직접 입력 · 팔로잉 목록 불러오기 · 받은 글 붙여넣기가 한데 모임
- 받은 글 붙여넣기가 버튼을 눌러 여는 방식에서 입력창이 늘 보이는 방식으로 바뀜
- `현재 목록 저장`을 왼쪽 시청 목록 위로 옮김. 저장 대상 바로 옆에서 누르게 됨
- 저장 창이나 버퍼링 안내가 떠도 팝업 높이가 변하지 않도록 정리

### v3.0.1

- SOOP 방송을 좁은 칸에 넣으면 화면이 잘리고 왼쪽에 빈 여백이 생기던 문제 수정. 칸이 좁을 때는 방송 화면 전체가 비율 그대로 줄어들어 들어감
- 팝업 `기타설정`의 첫 설정 이름을 `서브채널 정보 표시`에서 `방송 칸 채널 표시`로 변경. 무엇을 조절하는 설정인지 이름만 보고 알 수 있도록 함

### v3.0.0

**화면 배치를 직접 구성하는 방식으로 전면 개편**

- 미리 정해진 배치 4종(기본형/반전형/서브하단/서브상단) 중 고르던 방식을 없애고, 화면을 원하는 대로 나눠 쓰도록 바꿈
- 메인 화면과 서브 화면 구분을 없앰. 모든 채널이 같은 크기의 칸 하나를 차지하고, 원하는 칸만 크게 키워서 볼 수 있음
- 칸 사이 경계를 끌어 크기 조절
- 칸 오른쪽 위의 손잡이를 잡아 다른 칸 가운데에 놓으면 자리 교환, 가장자리에 놓으면 그 칸이 갈라지면서 들어감
- 구성한 배치가 저장되어 다시 열어도 그대로 유지되고, 창 크기를 바꿔도 비율이 유지됨
- 서브 영역을 화면의 40%까지만 넓힐 수 있던 제한이 사라짐

**그 밖의 변화**

- 자리를 바꿔도 방송이 끊기지 않음. 예전에는 메인과 서브를 교환할 때마다 화면을 다시 불러와 잠시 멈추고 소리·화면 크기가 풀렸음
- 팝업에서 채널 순서만 바꿔도 모든 방송이 다시 불러와지던 문제 해소
- 소리를 방송 화면 안의 볼륨 조절로 직접 다루도록 바꿈. 채널마다 마지막에 쓰던 크기로 시작함
- 대시보드 채팅 기능 제거
- 팝업에서 메인 화면 지정과 목록 순서 변경 제거. 화면상의 자리는 대시보드가 기억함
- 팝업 시청 목록에 즐겨찾기 별 버튼 추가 (등록 전용, 해제는 즐겨찾기 목록에서)
- 팝업 시청 목록에 생방송 여부(LIVE/OFF) 표시 추가
- 시청 채널이 6개 이상이면 네트워크 환경에 따라 버퍼링이 생길 수 있다는 안내 표시
- 즐겨찾기에서 채널을 지워도 시청 목록의 즐겨찾기 표시가 그대로 남던 문제 수정

### v2.2.0
- 메인 채널 볼륨이 프로그램 실행 시 100으로 강제 고정되던 문제 수정
- 팝업 멀티뷰 표시 목록에서 드래그앤드롭으로 채널 순서를 변경할 수 있는 기능 추가

### v2.1.1
- 서브 → 메인 스왑 후 새로고침 시 음량이 0으로 고정되는 버그 수정

### v2.1.0
- 광고 재생 중 자동 동기화가 새로고침을 실행하는 문제 수정 (치지직·SOOP 공통)
- 치지직 API 데이터 추출 방식 개선 — 더 안정적인 방식으로 전환
- 서브 채널 영역 확대 보기 추가 — 버튼 클릭 후 드래그로 원하는 영역만 확대, 버튼으로 원래 화면 복원 가능
- 서브 채널 순서 변경 추가 — 서브 타일을 드래그해서 원하는 순서로 재배치
- 서브 채널 가로 배치 시 마우스 휠 스크롤로 목록 좌우 이동 지원

### v2.0.0
- 프로그램명 MultiStream으로 변경
- SOOP(아프리카TV) 채널 멀티뷰 지원 추가
- 팝업 UI 전면 개편 — 시청목록 / 기타설정 2탭 구조로 통합
- 대시보드 표시 목록에서 멀티뷰 목록과 즐겨찾기를 나란히 표시
- 팔로잉 목록 및 즐겨찾기에 치지직 / SOOP 플랫폼 아이콘 표시
- LIVE 배지 위치 개선 — 이름보다 앞에 표시
- 확장 아이콘 및 UI 색상 전면 교체

### v1.5.0
- 방송 중이 아닌 서브채널에서도 삭제 버튼이 항상 표시되도록 개선
- 서브채널 정보 표시 방식 선택 기능 추가 (프로필 사진 또는 채널 이름, 항상 표시 / 마우스 올릴 때 중 선택)
- 치지직 외 페이지(스튜디오, 드롭스 등) 방문 시 오류 발생하던 문제 수정
- 다른 앱 작업 중 브라우저로 전환 시 서브채널이 새로고침되던 문제 수정

### v1.4.0
- 즐겨찾기 목록에 폴더(트리) 구조 추가 (드래그앤드롭으로 항목/폴더 정리 가능)
- 즐겨찾기 항목에 현재 생방송 여부(LIVE/OFF) 표시 추가
- 시청 목록에서 항목별 메뉴(⋯)로 즐겨찾기에 바로 추가할 수 있도록 개선
- 서브 화면 좌측 상단에 채널 프로필 사진 표시 (기타 설정에서 항상 표시 / 마우스 올릴 때만 표시 / 표시 안함 선택 가능)
- 기본형/반전형 레이아웃에서 서브 화면 영역을 더 넓게 조절할 수 있도록 개선

### v1.3.0
- 와이드 화면 자동 전환이 간헐적으로 동작하지 않던 문제 개선 (화면 교체 시에도 재시도)
- 시청 목록 채널이 방송 중이 아닐 때 "방송중이 아닙니다" 안내 표시 (1분마다 재확인 후 방송 재개 시 자동 복귀)
- 화면을 새로 불러올 때 "초기화 진행중입니다" 안내 표시
- 서브 화면에도 자동 동기화 적용 (영상 신호 없을 때 자동 재시도 포함)
- 서브 화면을 메인으로 전환할 때 간헐적으로 소리가 나오지 않던 문제 수정
- "전체 새로고침" 버튼 제거 (필요 시 브라우저 새로고침 이용)
- 시청 중인 채널 수 표시 단위를 "명"에서 "채널"로 변경
- 시청 목록에 스트리머를 직접 추가할 때 탭이 자동으로 전환되지 않도록 변경
- 팔로잉 목록에서 즐겨찾기에 등록된 항목 표시를 "저장됨"에서 "즐겨찾기"로 변경
- 즐겨찾기 목록에 LIVE/OFF 표시 추가

### v1.2.1
- 확장 프로그램 이름 표기 오류 수정

### v1.2.0
- 메인 채널 지연시간이 기준치를 초과해도 새로고침되지 않던 문제 수정
- 대시보드를 새로고침해도 팝업에서 변경한 레이아웃이 적용되도록 개선
- 대시보드가 이미 열려있을 때 팝업에서 다시 열면 기존 창으로 이동하도록 개선
- 팝업 알림을 화면 하단 메시지 형태로 개선
- 시청 목록에서 스트리머 순서를 위아래로 변경할 수 있는 기능 추가
- 채널 ID / 스트리머명 입력 시 특수문자 입력 제한 추가
- 즐겨찾기 목록에서 이미 시청 목록에 추가된 채널은 "추가됨"으로 표시
- 치지직에 로그인되어 있지 않을 때 팔로잉 목록을 불러오지 못하던 오류 수정 (로그인 안내 화면 추가)

### v1.1.0
- 레이아웃 선택을 시청 목록 탭으로 이동 (대시보드 열기 버튼 하단)
- 기타 설정 탭 하단 버전 번호 표시 추가
- 레이아웃 3/4 서브채널 높이 드래그 조절 기능 추가 (저장/복원)
- 코드 구조 모듈화 리팩터링 (popup/, dashboard/ 폴더 분리)
- 리소스 파일 구조 정리 (resources/ 폴더로 이동)

### v1.0.3
- 레이아웃 4종 선택 기능 추가 (서브 좌/우/하단/상단)
- 대시보드에서 서브채널 개별 삭제 기능 추가
- 메인/서브 채널 딜레이 실시간 표시
- 서브 채널 최대 너비 제한 조정
- 생방송 시 와이드 모드 자동 전환 개선

### v1.0.2
- 방송 딜레이 표시 기능 추가
- 초기 멀티뷰 기본 기능 안정화

### v1.0.1
- 초기 릴리즈: 멀티뷰 대시보드, 팔로잉 연동, 즐겨찾기
