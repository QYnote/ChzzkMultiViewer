...
`document/Overview.md`는 사용자 관점으로 쓴다. 별도 지시가 없으면 개발자 전용(내부 구현) 내용은 포함하지 않고, 코드 내부 구현 용어(파일명, 함수명, 변수명, CSS 클래스명 등)도 사용하지 않는다.

모듈별 문서(`Popup.md` 등)와 `Architecture.md`는 개발자 관점 문서이므로, 계약을 설명하는 데 필요한 식별자(메시지 이름, 스토리지 키, API 이름 등)는 그대로 쓴다.
