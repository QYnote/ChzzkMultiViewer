...
- [x] 기준 허용 지연시간 초과 시 새로고침되지 않는 문제 수정
- [x] 대시보드 새로고침 시 팝업에서 변경된 레이아웃 적용
- [x] 대시보드가 이미 열려 있을 때 팝업에서 클릭 시 새 탭 대신 기존 탭으로 포커스 이동
- [x] 팝업 내 alert() 를 인라인 알림 UI로 교체
- [x] 시청 목록 순서 변경 기능 추가 (위아래 이동 버튼)
- [x] 스트리머 이름 innerHTML 삽입을 textContent 방식으로 교체
- [x] 수동 입력 폼(채널 ID, 스트리머명)에 HTML 특수문자 입력 차단 처리 추가
- [ ] 팝업에서 즐겨찾기 목록을 시청 목록 하단에 위치하도록 UI 변경
- [ ] 다른 앱 작업 중 브라우저로 포커스가 이동할 때 서브채널이 새로고침되는 문제 수정
- [ ] 와이드 모드 자동 전환이 간헐적으로 동작하지 않는 문제 수정
- [ ] 통나무 파워 자동 수령 — 채팅창에 수령 버튼이 나타나면 자동으로 클릭
- [ ] 다른 PC에서 다른 유저로 로그인 시 "로그인이 만료되었습니다. 새로고침 해주세요" 메시지가 사라지지 않는 문제 수정
- [x] 즐겨찾기 목록에서 시청 목록에 이미 추가된 채널은 팔로잉 목록처럼 비활성화 상태로 표시
