...
- [ ] 메인↔서브를 교환한 뒤 새 메인 화면이 음소거 상태로 남는 버그 수정 — 새 메인은 직전 메인의 볼륨 수준을 이어받는 구조라, 이어받은 값이 0이면 소리가 나오지 않음. v1.3.0("서브 화면을 메인으로 전환할 때 간헐적으로 소리가 나오지 않던 문제")과 v2.1.1("서브 → 메인 스왑 후 새로고침 시 음량이 0으로 고정")에서 유사 증상을 고친 이력이 있어 재발 여부부터 확인 필요
