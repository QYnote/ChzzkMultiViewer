...
- [ ] 와이드 모드 자동 전환이 간헐적으로 동작하지 않는 문제 수정
- [ ] ~~통나무 파워 자동 수령 — 채팅창에 수령 버튼이 나타나면 자동으로 클릭~~ — **개발 제외**: 네이버에서 작업장(매크로)으로 판단할 수 있는 위험이 있어 보류
- [ ] 아프리카TV(SOOP) 채널 멀티뷰 지원 추가
- [ ] 시청 목록 채널이 생방송 중이 아닐 경우 메인/서브 화면에 "방송중이 아닙니다" 표시 (1분 주기 재확인)
- [ ] 서브채널 자동동기화 적용 토글 추가
- [ ] 대시보드 새로고침/초기 로딩 시 화면에 "초기화 진행중입니다" 안내 표시 (이전의 진한 반투명 오버레이 스타일 참고)
