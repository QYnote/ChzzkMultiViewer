...
- [x] 서브 채널 영역 확대 보기 — 버튼 클릭 후 드래그로 영역 선택, 해당 부분만 확대 표시 / 확대 중 버튼으로 원래 화면 복원 가능
- [x] 서브 채널 순서 변경 — 서브 타일을 드래그해서 원하는 순서로 재배치
- [x] 서브 채널 가로 배치(상단/하단 레이아웃) 시 마우스 휠 스크롤로 목록 좌우 이동 — 아래 스크롤 시 우측, 위 스크롤 시 좌측
