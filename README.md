...
- **프로필 사진 표시** — 서브 타일 좌측 상단에 채널 프로필 사진 표시, 항상 표시/마우스 올릴 때만 표시/표시 안함 중 선택 (기타 설정)
...
- [x] 서브채널 높이의 1/4 크기로 채널 프로필 사진 표시 (최대 크기는 기본 프로필 사진 크기로 제한)
- [ ] 다른 앱 작업 중 브라우저로 포커스가 이동할 때 서브채널이 새로고침되는 문제 수정
- [ ] 아프리카TV(SOOP) 채널 멀티뷰 지원 추가
- [x] 시청 목록에 등록된 스트리머를 즐겨찾기에도 추가할 수 있는 기능 추가 (버튼 증가에 따른 표시 UI 추가 구상 필요)
- [x] 즐겨찾기 목록에 폴더(트리) 구조 추가 — 팔로잉 목록에서 추가 시 기본적으로 Root에 추가, 즐겨찾기 페이지에서 드래그앤드롭으로 폴더 하위로 이동
