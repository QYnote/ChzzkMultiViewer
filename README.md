...
- [ ] 서브채널 방송 중이 아닐 때도 해당 서브채널 삭제 가능하도록 수정
- [ ] 서브채널 프로필 사진이 채널명을 가리지 않도록 채널명 옆(같은 줄)에 표시되도록 수정
- [x] 해당프로그램이 활성화된 상태에서 스튜디오, 드롭스페이지 등에 접속 시 페이지에 오류가 생기는 문제 수정
- [x] 다른 앱 작업 중 브라우저로 포커스가 이동할 때 서브채널이 새로고침되는 문제 수정
